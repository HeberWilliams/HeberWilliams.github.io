document.addEventListener('DOMContentLoaded', () => {
    ...
    let scoreDisplay = document.querySelector('.container')
...
// generate platform at random location
    ...
    function gameOver() {
        ...
        grid.insertAdjacentHTML('afterend', 'press any key to reset')
    ...
    function moveStraight() {
        isGoingLeft = false
        isGoingRight = false
        clearInterval(righTimerId)
        clearInterval(leftTimerId)
    }

    function clearInitialText() {
        if (gameStarted) {
            document.querySelector(".initial h1").classList.add('hide')
            document.querySelector(".initial h4").classList.add('hide')
            document.querySelector(".initial img").classList.add('hide')
    ...
    //attach to button and key spacebar
    document.addEventListener('keyup', event => {
        if (event.code === 'Space') {
